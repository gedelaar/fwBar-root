...
class LedenModel extends Model {
    ...
    function leden_getall() {
        //$query = $db->table('leden')->get();
        
        //$db->reconnect();
        $builder = $this->db->table('leden');
        $query   = $builder->get();
        return $query->getresult();
    }

    function leden_nobar($id, $status) {
        $this->id = $id;
        ...
        if ($query->num_rows() > 0) {
            $this->get_bar_from_id();
            if ($status == '8') {
                $this->bar_statusupdate($status, 0, 0, $this->lidnummer);
                $this->add_bardienst(0, $this->poule, $this->code, "");
            } else {
                if ($this->dienst <> '9') {
                    $this->bar_statusupdate($status, $this->poule, $this->code, $this->lidnummer);
                    $this->add_bardienst(0, $this->poule, $this->code, "");
                }
            }
        }
        $this->vul_bar_init();
        $this->vul_opendiensten_in();
        return $query->result();
    }

    function leden_yesbar($id, $status) {
        $this->id = $id;
        $this->db->from("bardienst");
        $this->db->join("wedstrijden", "bardienst.poule=wedstrijden.poule and bardienst.code=wedstrijden.code");
        $this->db->where('id', $this->id);
...
//            debug_var($this->dienst);
    ...
    function zoek_lid_zonder_bardienst() {
        ...
        $query = $this->db->get();
        //debug_log("function=zoek_lid_zonder_bardienst");
        //debug_last_query();
        //debug_query_result($query);
        // die;
...
//        debug_query_result($query);
    ...
    function mail_body() {
        ...
        //$strBody = '<br>Bardienst opnieuw ingedeeld!!!!!';
        ...
        $strBody.= "<br><br>";
        $strBody.= anchor("http://www.forwodians.nl/fwBar/Handleiding_bardienst_2019-2020v1.pdf", 'handleiding bardienst');
        $strBody.= "<br>";
    ...
    function mail_official_selection() {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $rij) {
				...
				$list=array();
				if(!is_null($rij->eml)){$list = $this->add_to_arr($list,$rij->eml);}
				if(!is_null($rij->fml)){$list = $this->add_to_arr($list,$rij->fml);}
				if(!is_null($rij->gml)){$list = $this->add_to_arr($list,$rij->gml);}				
				if(!is_null($rij->hml)){$list = $this->add_to_arr($list,$rij->hml);}				
				if(!is_null($rij->iml)){$list = $this->add_to_arr($list,$rij->iml);}				
				if(!is_null($rij->jml)){$list = $this->add_to_arr($list,$rij->jml);}				
				if(!is_null($rij->yml)){$list = $this->add_to_arr($list,$rij->yml);}				
				if(!is_null($rij->zml)){$list = $this->add_to_arr($list,$rij->zml);}				
                ...
                $this->email->from('send@forwodians.nl', 'wedstrijdCie Forwodians');
                $this->email->reply_to('indeling@forwodians.nl');
                //        $this->email->cc('another@another-example.com');
                $bcc_list = array('fw_check@edelaar.nl','forwodians.basketbal@gmail.com');
	...
	function add_to_arr($arr, $element){
		//echo "hier";
		$hlp_element = str_replace(";","|",str_replace(",","|",$element));
		$hlp_arr = explode("|",$hlp_element);
...
//                $list = array($rij->eml, $rij->fml, $rij->gml, $rij->hml, $rij->iml, $rij->jml, $rij->yml, $rij->zml);
    ...
    function mail_body_tafelaars() {
        ...
        $strBody.= "<br><br>";
		$strBody.= "Vergeet niet je eigen fluit mee te nemen.";
		$strBody.= "Heb je geen fluit of geen scheidsrechtershirt dan kan die op website bestellen en dat ligt dan voor je klaar in de kantine.";
		$strBody.= "<br><br>";
        ...
        //$strBody.=  "poule = " . $this->poule."<br><hr><br>";   
        //echo "<br>".$strBody;
		$strBody.="<br>Wil je nog extra begeleiding bij het uitvoeren van je scheidsrechter- of tafelbeurt laat het weten via tc@forwodians.nl";
        return $strBody;
    }
	...
	//echo dirname(__FILE__);die;
		...
		$objPHPExcel = $objReader->load( '/www/temp/Leden.xls' );
